    ...
    def submit():
        ...
        for i in js.keys():
            if js[i]["for"] == y:
                listbox.insert(
                    END,
                    Button(listbox, text=str(js[i]), command=partial(click, i)).pack(),
                )
...
User = Button(
    root, text="Worker", padx=5, pady=3, command=userLogin, font=("Arial", 25)
)
Employer = Button(
    root, text="Employer", padx=5, pady=3, command=employerLogin, font=("Arial", 25)
)
User.pack()
Employer.pack()
